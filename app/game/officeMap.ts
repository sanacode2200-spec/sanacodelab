...
function cyl(r1: number, r2: number, h: number, color: number, segments = 24) {
  return new THREE.Mesh(new THREE.CylinderGeometry(r1, r2, h, segments), toonMaterial(color));
}
...
export function addOfficeMap(scene: THREE.Scene) {
  ...
  for (const desk of DESK_PLACEMENTS) {
    ...
    const table = box(2.4, 0.28, 1.15, hex(F.table));
    table.position.y = 0.75;
    base.add(table);
    const monitor = box(0.72, 0.48, 0.08, hex(F.monitor));
    monitor.position.set(0, 1.15, -0.35);
    base.add(monitor);
    const monitor2 = box(0.58, 0.4, 0.08, hex(F.monitor));
    monitor2.position.set(0.55, 1.1, -0.28);
    monitor2.rotation.y = -0.22;
    base.add(monitor2);
    const chair = box(0.72, 0.62, 0.65, hex(F.chair));
  ...
  const table = cyl(1.85, 1.85, 0.3, hex(F.table), 36);
  table.position.y = 0.68;
  meeting.add(table);
  for (let i = 0; i < 6; i++) {
    const chair = box(0.55, 0.48, 0.55, hex(F.chair));
  ...
  const counter = box(2.9, 0.85, 0.75, hex(F.counter));
  counter.position.y = 0.55;
  lounge.add(counter);
  const sofaA = box(1.65, 0.55, 0.72, hex(F.sofa));
  sofaA.position.set(-1.5, 0.43, 1.25);
  lounge.add(sofaA);
  const sofaB = box(1.65, 0.55, 0.72, hex(F.sofa));
  ...
  for (const [radiusDeg, angleDeg] of plantPoints) {
    ...
    const pot = cyl(0.28, 0.34, 0.38, hex(theme.plant.pot), 12);
    pot.position.y = 0.22;
    plant.add(pot);
    const leaves = new THREE.Mesh(new THREE.SphereGeometry(0.55, 14, 10), toonMaterial(hex(theme.plant.leaves)));
  ...
  // 家具一式にアウトラインを付けて原神風のくっきりした輪郭にする
  addOutlineDeep(furniture);
  return furniture;
}
